...
type Mapping = { publicModel: string; upstreamModel: string };
type Channel = {
  id: string; name: string; provider: string; baseUrl: string; status: string; weight: number; priority: number; rpmLimit: number; tpmLimit: number;
...
export function ChannelManager() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <Table>
        <THead><TR><TH>名称</TH><TH>提供商</TH><TH>状态</TH><TH>模型映射</TH><TH>Keys</TH><TH>限流</TH><TH className="text-right">操作</TH></TR></THead>
        <TBody>
          {rows.map((c) => (
            <TR key={c.id}>
              ...
              <TD className="font-mono text-xs">{c.modelMappings.map((m) => <div key={m.publicModel}>{m.publicModel} → {m.upstreamModel}</div>)}</TD>
              <TD>
                {c.keys.map((k) => (
          ...
          {!rows.length && <TR><TD colSpan={7} className="py-8 text-center text-muted-foreground">暂无渠道</TD></TR>}
        </TBody>
      </Table>
    </div>
  );
